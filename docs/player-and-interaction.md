...
Land vertical motion integrates constant gravity analytically from the elapsed frame time, preserving the authored jump arc across render rates. The engine clamps a frame delta to 100 ms, and the controller’s collision sweep must still allow a one-block ledge to be cleared at that maximum interval.
