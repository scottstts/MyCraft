...
Before the visible `RenderPass`, the composer updates the water-free scene/depth capture, direct voxel sun visibility, a forward-refracted RGB32F source-receiver field, matched world-space visibility for those receivers, and finally the forward water-interface radiance layer. The last resource is not a second ordinary scene capture. Participating source materials run a forward Fermat/Snell vertex projection and rasterize their opposite-medium silhouette directly into transparent color/depth targets. The visible ocean then samples that layer at the same framebuffer coordinate, so refraction cannot separate color from coverage or create a second outline around submerged geometry. Forward shadow filtering is gated by source-world receiver continuity and uses the refracted source Jacobian for its pixel footprint; the ordinary direct-shadow path is unchanged.
