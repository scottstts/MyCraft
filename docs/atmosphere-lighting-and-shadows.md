...
The occupancy volume is updated from `World` chunk and block events, not from mesh arrival. This keeps shadows tied to gameplay state after mining, placing, chunk replacement, and save ingestion. The animated player is supplied separately as exact shadow boxes each frame, so the character does not need to be rasterized into the terrain occupancy volume. Leaf cells stay out of solid occupancy: mixed and sparse leaf bricks trace the exact procedural leaf atlas alpha, while dense leaf-only 8³ bricks are integrated from a measured density field to avoid a per-fragment 8³ walk through a canopy. A fixed nine-ray solar-disc kernel keeps the work and result temporally stable as leaf edges move across pixels. Seaweed remains outside gameplay occupancy and contributes a compact render-only XZ caster field that the same DDA pass expands into a block-local crossed-blade proxy. The pass performs that lookup only for underwater receivers and reads one root cell per traversed DDA cell; it does not scan a 3×3 neighborhood. Both authoritative terrain and the render-only seabed/seaweed receivers share the same visibility-mask, receiver-depth, and resolution uniforms, which keeps the character and vegetation shadows continuous when the player swims beyond the authored terrain boundary.
