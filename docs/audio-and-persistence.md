...
Underwater ambience is gated by the active gameplay camera, not the player's physics head or swim state. The camera is compared with the live displaced ocean surface, and a 0.4-block audio envelope enables the underwater loop only when more than 50% is submerged; the ocean loop uses the same camera result for its underwater dimming. Character switching plays the supplied `src/assets/sounds/sound_effects/switch_sound.mp3` one-shot at the shared SFX volume.
