...
export class FogPass extends ShaderPass {
  constructor(){
    super({
      ...
      fragmentShader: `
        ...
        uniform float contrast; uniform float saturation;
        ...
        void main(){ vec3 col = texture2D(tDiffuse,vUv).rgb; float d = readDepth(vUv); if(enabled){ bool bg = d >= cameraFar*0.99; d = min(d, maxDistance); float f = 1.0 - exp(-d * baseDensity); if (bg) { f *= mix(0.3, 1.0, clamp(dayLight,0.0,1.0)); } col = mix(col, fogColor, clamp(f,0.0,0.9)); } col = adjustColor(col, contrast, saturation); gl_FragColor = vec4(col,1.0);} 
      `
    })
  }
  ...
  setColorGrading({ contrast, saturation }: { contrast?: number; saturation?: number }){ if(contrast!==undefined) this.uniforms.contrast.value = contrast; if(saturation!==undefined) this.uniforms.saturation.value = saturation }
}
