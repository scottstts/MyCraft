  ...
  constructor(
    ...
    const fragmentShader = `
      ...
      float getVoxelShadowMask() {
        ...
        vec2 uv = (gl_FragCoord.xy + vec2(0.5)) / max(voxelShadowResolution, vec2(1.0));
        float center = sampleVoxelShadow(uv);
        // Multi-ray visibility is quantized by design. Reconstruct only its
        // fractional edge signal from immediate neighbours; values near 0 or
        // 1 (contact umbra/full light) remain untouched, so this cannot wash
        // out hard contact shadows or leak across stable regions.
        ...
        float neighbours = (
          sampleVoxelShadow(uv + vec2(texel.x, 0.0)) +
          sampleVoxelShadow(uv - vec2(texel.x, 0.0)) +
          sampleVoxelShadow(uv + vec2(0.0, texel.y)) +
          sampleVoxelShadow(uv - vec2(0.0, texel.y))
        ) * 0.25;
        return mix(center, neighbours, 0.35 * uncertainty);
      }
